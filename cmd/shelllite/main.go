...
import (
	...
	"ssh-go/internal/config"    // 本项目：配置加载（JSON）
	"ssh-go/internal/db"        // 本项目：SQLite 数据库操作
	"ssh-go/internal/gui"       // 本项目：图形界面（Gio）
	"ssh-go/internal/logging"   // 本项目：简单日志封装
	"ssh-go/internal/sshclient" // 本项目：SSH 连接和命令执行
)
